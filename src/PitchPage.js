              ...
              <div className="col-lg-5">
                <div className="card-body" id="originCard{this.state.id}">
                  <div className="form-group">
                    ...
                    <select value={this.state.implementation} onChange={this.handleChange}>
                      {PageTypes.map((type, index) => (<option key={index} value={type}>{type}</option>))}
                    </select>
                  </div>
                  <div className="form-group">
                    <label for="pageTitle{this.state.id}">Page title</label> <input name="title" type="text" className="form-control pageTitle" id="pageTitle{this.state.id}" placeholder="Enter page title" required="" onChange={this.handleInputChange} value={this.state.title }/>
                    <div className="invalid-feedback">Please provide a valid title for this page.</div>
                  </div>
                  <div className="form-group">
                    <label for="content">Page content</label>
                    <textarea className="form-control pageContent" id="content{this.state.id}" rows="10">{this.state.text }</textarea>
                  </div>
